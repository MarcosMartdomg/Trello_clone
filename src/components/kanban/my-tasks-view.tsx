...
import { Calendar, Clock, Layout, ChevronRight, MessageSquare, User } from "lucide-react"
...
export function MyTasksView() {
    ...
    const tasksPerBoard = boards.map(board => {
        const filteredTasks = board.columns.flatMap(col =>
            col.cards.filter(card => {
                // Constraint: Only show cards that have at least one checklist item.
    ...
    const personalBoards = tasksPerBoard.filter(b => b.type === 'personal')
    const sharedBoards = tasksPerBoard.filter(b => b.type !== 'personal')
            ...
            {tasksPerBoard.length === 0 ? (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
                    ...
                    <div className="space-y-8">
                        <div className="flex items-center gap-3 px-1 pb-2 border-b border-border/10">
                            <div className="w-10 h-10 rounded-xl bg-violet-500/10 flex items-center justify-center">
                                <User className="w-5 h-5 text-violet-500" />
                            </div>
                            <h2 className="text-xl font-black uppercase tracking-widest text-foreground/80">{t('sidebar.personalBoards')}</h2>
                        </div>

                        {personalBoards.length === 0 ? (
                            <div className="p-12 text-center bg-card/30 rounded-3xl border border-dashed border-border/50">
                                <p className="text-sm text-muted-foreground font-medium italic">{t('board.noTasksAssigned')}</p>
                            </div>
                        ) : (
                    ...
                    <div className="space-y-8">
                        <div className="flex items-center gap-3 px-1 pb-2 border-b border-border/10">
                            ...
                            <h2 className="text-xl font-black uppercase tracking-widest text-foreground/80">{t('sidebar.sharedBoards')}</h2>
                        </div>
    ...
    function BoardSection({ board }: { board: any }) {
        return (
                    ...
                    {board.tasks.map((task: any) => (
                        <div
                            ...
                            className={cn(
                                ...
                                task.checklist?.every((i: any) => i.completed) && task.checklist?.length > 0 && "opacity-75 bg-secondary/20"
                            )}
                        >
                                ...
                                <div className="flex items-start gap-3 flex-1">
                                    ...
                                    >
                                        <div className={cn(
                                            ...
                                            task.checklist?.every((i: any) => i.completed) && task.checklist?.length > 0
                                    ...
                                    <div className="space-y-1">
                                        ...
                                        <h4 className={cn(
                                            ...
                                            task.checklist?.every((i: any) => i.completed) && task.checklist?.length > 0 && "line-through text-muted-foreground opacity-50"
                                        )}>
                                            {task.title}
                                        </h4>
                                    </div>
                                ...
                                <div className="flex items-center gap-4 text-muted-foreground">
                                    {task.due_date && (
                                        <div className={cn(
                                            ...
                                            task.checklist && task.checklist.length > 0 && task.checklist.every((i: any) => i.completed)
                                                ? "text-emerald-500"
